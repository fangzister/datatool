var UUEncode = {
    ...
    encode: function(inString) {
        ...
        var inBytes = Array.from(inString); //new Buffer(inString);
    ...
    decode: function(inString) {
        ...
        var inBytes = Array.from(inString); // new Buffer(inString);
        for (let i = 0, l = inBytes.length; i < l; i++) {
            inBytes[i] = inBytes[i].charCodeAt(0);
        }
        var buffLen = inBytes.length;
        ...
        var ret = outBytes.slice(0, totalLen);
