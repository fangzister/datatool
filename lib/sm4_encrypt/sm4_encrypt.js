    ...
    return function r(e, t, n) {
        ...
        "/": [function (r, e, t) {
            ...
            function d(r) {
                var e = r.length;
                if (e % 4 > 0) {
                    console.warn("解密失败字符串：" + sm4DecryptText);
...
function SM4() {
    ...
    this.sm4_crypt_cbc = function (ctx, iv, inputObject) {
...
function SM4Util() {
    ...
    this.encryptData_CBC = function (plainText) {
        try {
            //转为字符串
            plainText = JSON.stringify(plainText).replace(/\\/g, "");/*.trim()*/
            // plainText = String(plainText).trim();
            while (plainText.startsWith('"')) {
                plainText = plainText.slice(1, plainText.length - 1);
            }
            var sm4 = new SM4();
            var ctx = new SM4_Context();
            ctx.isPadding = true;
     ...
     * @param paramMap
     */
    this.encryptData_CBC_byMap = function (paramMap) {
        var sendParameters = $.extend({}, paramMap);
        if (sendParameters) {
            for (var item of Object.keys(sendParameters)) {
                sendParameters[item] = this.encryptData_CBC(sendParameters[item]);
            }
        }
    ...
    this.decryptData_CBC = function (cipherText) {
        try {
            ...
            // console.log(e);
            if (sm4DecryptText.indexOf("=") >= 0) {
                console.log("开始url参数未加密，将为url参数加密！");
            }
            return null;
        }
    };
